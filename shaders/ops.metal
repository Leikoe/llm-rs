...
{
    if (tid < dim) out[tid] = bfloat(table[token_id * dim + tid]);
}
...
kernel void embed_bf16(
    ...
    uint tid [[thread_position_in_grid]])
{
    if (tid < dim) out[tid] = bfloat(bf16_to_f32(table[token_id * dim + tid]));
}
...
kernel void embed_f16(
    ...
    uint tid [[thread_position_in_grid]])
{
    if (tid < dim) out[tid] = bfloat(float(table[token_id * dim + tid]));
}
    ...
    uint tid [[thread_position_in_grid]])
{
    ...
    out[tid] = bfloat(d * sm.x * q - dmin * sm.y);
}
    ...
    uint tid [[thread_position_in_grid]])
{
    ...
    out[tid] = bfloat(d * sc * (float(q) - 32.0));
}
    ...
    uint row [[thread_position_in_grid]])
{
    ...
    uint base = row * in_features;
    for (uint i = 0; i < in_features; i++) {
        sum += bf16_to_f32(weight[base + i]) * float(input[i]);
    }
    output[row] = bfloat(sum);
}
    ...
    uint row [[thread_position_in_grid]])
{
    ...
    uint base = row * in_features;
    for (uint i = 0; i < in_features; i++) {
        sum += float(weight[base + i]) * float(input[i]);
    }
    output[row] = bfloat(sum);
}
    ...
    uint row [[thread_position_in_grid]])
{
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        for (uint j = 0; j < 16; j++) {
            ...
            uint idx = b * 32 + j * 2;
            sum += scale * (float(lo) * float(input[idx]) + float(hi) * float(input[idx + 1]));
        }
    }
    ...
    uint row [[thread_position_in_grid]])
{
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        for (uint j = 0; j < 32; j++) {
            ...
            sum += scale * q * float(input[b * 32 + j]);
        }
    }
    ...
    uint row [[thread_position_in_grid]])
{
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        for (uint i = 0; i < 256; i++) {
            ...
            sum += (d * sm.x * q - dmin * sm.y) * float(input[b * 256 + i]);
        }
    }
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        for (uint sb = 0; sb < 16; sb++) {
            ...
            for (uint j = 0; j < 16; j++) {
                ...
                sum += d_sc * (float(q) - 32.0) * float(input[b * 256 + elem]);
            }
        }
    }
    ...
    threadgroup float* shared  [[threadgroup(0)]])
{
    ...
    for (uint i = tid; i < dim; i += tg_size) {
        output[i] = bfloat(float(input[i]) * inv_rms * weight[i]);
    }
}
    ...
    uint2 tgid [[threadgroup_position_in_grid]])
{
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        uint byte_idx = lane / 2;
        uchar packed = weight[bo + 2 + byte_idx];
        float w = scale * float((lane % 2 == 0) ? (int(packed & 0x0F) - 8) : (int(packed >> 4) - 8));
        for (uint ds = 0; ds < tile; ds++)
            sumf[ds] += w * float(input[(s_start + ds) * in_features + b * 32 + lane]);
    }
    for (uint ds = 0; ds < tile; ds++) {
        float t = simd_sum(sumf[ds]);
        if (lane == 0) output[(s_start + ds) * out_features + row] = bfloat(t);
    ...
    uint2 tgid [[threadgroup_position_in_grid]])
{
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        ushort scale_bits = ushort(weight[bo]) | (ushort(weight[bo + 1]) << 8);
        float scale = float(as_type<half>(scale_bits));
        float w = scale * float(as_type<char>(weight[bo + 2 + lane]));
        for (uint ds = 0; ds < tile; ds++)
            sumf[ds] += w * float(input[(s_start + ds) * in_features + b * 32 + lane]);
    }
    for (uint ds = 0; ds < tile; ds++) {
    ...
    uint2 tgid [[threadgroup_position_in_grid]])
{
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        for (uint pair = 0; pair < 4; pair++) {
            ...
            for (uint ds = 0; ds < tile; ds++) {
                uint si = (s_start + ds) * in_features + input_base;
                sumf[ds] += w_lo * float(input[si + pair * 2 * 32 + lane])
                          + w_hi * float(input[si + (pair * 2 + 1) * 32 + lane]);
            }
        }
    }
    for (uint ds = 0; ds < tile; ds++) {
    ...
    uint2 tgid [[threadgroup_position_in_grid]])
{
    ...
    for (uint b = 0; b < blocks_per_row; b++) {
        ...
        for (uint k = 0; k < 8; k++) {
            ...
            float sc = float(as_type<char>(scales[elem / 16]));
            uchar q = dequant_q6k_elem(ql, qh, elem);
            float w = d * sc * (float(q) - 32.0);
            for (uint ds = 0; ds < tile; ds++)
                sumf[ds] += w * float(input[(s_start + ds) * in_features + input_base + k * 32 + lane]);
        }
    }
    for (uint ds = 0; ds < tile; ds++) {
        float t = simd_sum(sumf[ds]);
